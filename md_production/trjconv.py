...
def trjconv(setting):
    ...
    bash('gmx_mpi trjconv -f prod.xtc -o prodnj1.xtc -s min1.tpr -n index.ndx -pbc nojump << EOF\n System\n EOF')
    bash('gmx_mpi trjconv -f prodnj1.xtc -o prodnj2.xtc -s min1.tpr -n index.ndx -center <<EOF\n Protein\n System\n EOF')
    bash('gmx_mpi trjconv -f prodnj2.xtc -o prodnj3.xtc -s min1.tpr -n index.ndx -ur rect -pbc mol <<EOF\n System\n EOF')
    bash('gmx_mpi trjconv -f prodnj3.xtc -o prodnj4.xtc -s min1.tpr -n index.ndx -fit rot+trans <<EOF\n C-alpha\n System\n EOF')
